...
#define ADC_OFFSET                0         /* ADC scale offset */
#define ADC_SCALE                 7         /* ADC amplification scale */
 ...
 */
void example_i2s_adc_data_scale(uint8_t * d_buff, uint8_t* s_buff, uint32_t len, uint32_t scale_bit)
{
    uint32_t j = 0;
    uint32_t dac_value = 0;
#if (EXAMPLE_I2S_SAMPLE_BITS == 16)
    example_disp_buf((uint8_t*) s_buff, 32);
    for (int i = 0; i < len; i += 2) {
        dac_value = ((((uint16_t) (s_buff[i + 1] & 0xf) << 8) | ((s_buff[i + 0]))));
        dac_value = ((dac_value - ADC_OFFSET) << scale_bit) +  ADC_OFFSET;   // amplify 2^scale_bit  
    ...
    for (int i = 0; i < len; i += 4) {
        ...
        dac_value = ((((uint16_t) (s_buff[i + 1] & 0xf) << 8) | ((s_buff[i + 0]))));
        dac_value = ((dac_value - 0x8000) << scale_bit) + 0x8000;   // amplify 2^scale_bit 
        d_buff[j++] = 0;
        d_buff[j++] = 0;
        d_buff[j++] = dac_value & 0xff;
        d_buff[j++] = (dac_value >> 8 ) & 0xff;
...
void example_i2s_adc_dac(void*arg)
{
    ...
    while (1) {
        ...
        for (int rd_offset = 0; rd_offset < flash_wr_size; rd_offset += FLASH_SECTOR_SIZE) {
            ...
            example_i2s_adc_data_scale(i2s_write_buff, flash_read_buff, FLASH_SECTOR_SIZE, ADC_SCALE);
            //send data
